...
To register the docs.netapp.com MCP server with Cursor you must update the MCP configuration file. 
...
NOTE: If the file already has other servers configured, add `netapp-docs-mcp` inside the existing `mcpServers` object (make sure to add a comma after the previous entry). If there is no `mcpServers` key, add the key at the top level next to any other settings.
...
* The docs.netapp.com MCP server chooses an appropriate tool to provide results based on your query. 
