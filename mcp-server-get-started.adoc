...
NOTE: If the file already has other servers configured, add `netapp-docs-mcp` inside the existing `mcpServers` object ( adding a comma after the previous entry). If there is no `mcpServers` key, add the key at the top level next to any other settings.
